...
def writeBatch2ncfile(time_rec, zeta_rec, ix, iy):
    """ Write a BATCH of data to a NetCDF file """
    filename = fileNameFormat
    ncfile = nc4.Dataset(filename, mode="w", format="NETCDF4")
    ncfile.createDimension("y", ny)
    ncfile.createDimension("x", nx)
    ncfile.createDimension("lead_time", nSprints+1)
    ncfile.createDimension("batch", raceBatch)

    y_nc = ncfile.createVariable("y", np.float32, ("y",))
    y_nc.long_name = "y"
    x_nc = ncfile.createVariable("x", np.float32, ("x",))
    x_nc.long_name = "x"
    ibatch = ncfile.createVariable("batch", np.float32, ("batch",))
    ibatch.long_name = "batch"
    ilead = ncfile.createVariable("lead_time", np.float32, ("lead_time",))
    ibatch.long_name = "forecast lead time"
    
    itime = ncfile.createVariable("time", np.float32, ("batch", "lead_time",))
    itime.long_name = "time"

    zeta = ncfile.createVariable(
        "zeta", np.float32, ("batch", "lead_time", "y", "x"), fill_value=1.0e36
    )
    zeta.long_name = "vorticity"

    x_nc[:] = ix
    y_nc[:] = iy
    ibatch[:] = np.arange(0, raceBatch)
    ilead[:] = time_rec[0,:] - time_rec[0,0]
    itime[:, :] = time_rec
    zeta[:, :, :, :] = zeta_rec[:, :, :, :]
    ncfile.close()
    return filename
